import pandas as pd
import numpy as np
test_data = pd.read_csv('/Users/duanjingyuan/Documents/Fintech80 2021/data analysis/Online Data/test_data/test_data.csv')
company_base = pd.read_csv('/Users/duanjingyuan/Documents/Fintech80 2021/data analysis/Online Data/training data/corporate_attributes/company_base_info.csv')
matched_data = pd.merge(test_data,company_base,on='entid')
labels = pd.read_csv('/Users/duanjingyuan/Documents/Fintech80 2021/data analysis/Online Data/training data/label_risk_company.csv')
matched_labels = pd.merge(test_data,labels,on='entid')
matched_test = pd.merge(matched_labels,company_base,on='entid',how='left')
def base_process(original_data,filename):
    ...
    if 'CaseType' not in original_data.columns.values.tolist():
        casetype = []
        for i in range(len(original_data)):
            casetype.append(np.nan)
        reccap_data['CaseType'] = casetype
    #绘制大表
    ...
    return base_info
test_info = base_process(matched_data,'base_info.csv')
matched_info = base_process(matched_test,'label_base.csv')
